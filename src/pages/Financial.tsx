...
function freezeFirstFiveColumns(workbookData: Uint8Array): Uint8Array {
  const view = new DataView(
    ...
    workbookData.byteLength,
  );
  let endOfDirectory = -1;

  for (let offset = workbookData.length - 22; offset >= 0; offset--) {
  ...
  if (endOfDirectory < 0) return workbookData;

  const centralDirectoryOffset = view.getUint32(endOfDirectory + 16, true);
  const entries: Array<{
  ...
  while (
    centralOffset < endOfDirectory &&
    view.getUint32(centralOffset, true) === ZIP_CENTRAL_DIRECTORY_HEADER
  ) {
    ...
    entries.push({
      ...
      name: decoder.decode(workbookData.slice(nameStart, nameStart + fileNameLength)),
  ...
  const frozenPane =
    '<sheetViews><sheetView workbookViewId="0"><pane xSplit="5" topLeftCell="F1" activePane="topRight" state="frozen"/><selection pane="topRight" activeCell="F1" sqref="F1"/></sheetView></sheetViews>';
  ...
  entries
    ...
    .forEach((entry) => {
      const localView = new DataView(
        workbookData.buffer,
        workbookData.byteOffset + entry.localOffset,
        workbookData.byteLength - entry.localOffset,
      );
      if (localView.getUint32(0, true) !== ZIP_LOCAL_FILE_HEADER) return;
      ...
      const source = decoder.decode(
        workbookData.slice(dataStart, dataStart + entry.compressedSize),
      );
      const updated = source.replace(/<sheetViews>[\s\S]*?<\/sheetViews>/, frozenPane);
      replacements.set(entry.name, encoder.encode(updated));
    });

  if (!replacements.size) return workbookData;
  ...
  [...entries]
    ...
    .forEach((entry) => {
      const localHeader = workbookData.slice(entry.localOffset, entry.localOffset + 30);
      const localHeaderView = new DataView(localHeader.buffer, localHeader.byteOffset, 30);
      const localNameLength = localHeaderView.getUint16(26, true);
      const localExtraLength = localHeaderView.getUint16(28, true);
      const headerAndName = workbookData.slice(
        entry.localOffset,
        entry.localOffset + 30 + localNameLength + localExtraLength,
      );
      const contents = replacements.get(entry.name) ?? workbookData.slice(
        entry.localOffset + 30 + localNameLength + localExtraLength,
        entry.localOffset + 30 + localNameLength + localExtraLength + entry.compressedSize,
      );
      const patchedHeader = headerAndName.slice();
      const patchedHeaderView = new DataView(
        patchedHeader.buffer,
        patchedHeader.byteOffset,
        patchedHeader.byteLength,
      );

      if (replacements.has(entry.name)) {
        patchedHeaderView.setUint32(14, crc32(contents), true);
        patchedHeaderView.setUint32(18, contents.length, true);
        patchedHeaderView.setUint32(22, contents.length, true);
      }

      updatedOffsets.set(entry.name, localOffset);
      localParts.push(patchedHeader, contents);
      localOffset += patchedHeader.length + contents.length;
    });

  const centralParts = entries.map((entry) => {
    const nextOffset = entries.indexOf(entry) + 1 < entries.length
      ? entries[entries.indexOf(entry) + 1].centralOffset
      : centralOffset;
    const centralEntry = workbookData.slice(entry.centralOffset, nextOffset).slice();
    const centralEntryView = new DataView(
      centralEntry.buffer,
      centralEntry.byteOffset,
      centralEntry.byteLength,
    );
    const contents = replacements.get(entry.name);

    if (contents) {
      centralEntryView.setUint32(16, crc32(contents), true);
      centralEntryView.setUint32(20, contents.length, true);
      centralEntryView.setUint32(24, contents.length, true);
    }
    centralEntryView.setUint32(42, updatedOffsets.get(entry.name) ?? entry.localOffset, true);
    return centralEntry;
  });
  const centralDirectory = concatBytes(centralParts);
  const footer = workbookData.slice(endOfDirectory).slice();
...
function generateCommissionReportExcel(
  ...
  const wbout = freezeFirstFiveColumns(
